...
st.markdown("整合 **六級風險深度診斷**、**作業研究最佳化**、**長期趨勢預測** 與 **動態沙盤推演** 的決策支援系統。")
...
with tab1:
    ...
    with col2:
        ...
        st.info("系統運用 SLSQP 最佳化演算法，計算最大化整體健康度之預算配置。")
        m_cols = st.columns(2)
        for i, cat in enumerate(categories):
            delta = curr_scores[i] - prev_scores[i]
            with m_cols[i % 2]:
...
with tab2:
    ...
    with st.expander("📄 查看詳細數據表格"): st.dataframe(df_trend, use_container_width=True)
